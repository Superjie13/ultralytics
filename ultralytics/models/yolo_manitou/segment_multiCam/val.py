...
class ManitouSegmentationValidator_MultiCam(ManitouValidator_MultiCam):
    ...
    def preprocess(self, batch):
        ...
        for k in ["key_frames", "ref_frames"]:
            frames = batch.get(k, None)
            if frames is not None and "masks" in frames and frames["masks"] is not None:
                frames["masks"] = frames["masks"].to(self.device).float()
    ...
    def postprocess_forReid(self, preds, features):
        ...
        for b in range(len(p)):
            if p[b].shape[0] == 0:
                feat_preds.append(torch.empty((0, features[b].shape[0]), device=features.device))  # empty case
                continue
            masks = torch.einsum('in,nhw->ihw', p[b][:, 6:].to(proto[b].dtype), proto[b]).sigmoid()
            masks = crop_mask(masks, p[b][:, :4] / self.args.mask_ratio).unsqueeze(1)
            emb = (masks * features[b]).sum(dim=(2, 3)) / (masks.sum(dim=(2, 3)) + 1e-6)
            feat_preds.append(emb)

        return [p, proto], feat_preds
